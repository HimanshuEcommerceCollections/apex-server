...
export const updatePricingSchema = z
  .object({
    ...
    taxRateBps: z.coerce.number().int().min(0).max(MAX_TAX_BPS).optional(),
...
const groupBase = {
  ...
  unitPrice: z.coerce.number().int().min(0).max(MAX_CENTS).nullable().optional(),
};
...
export const createOptionSchema = z.object({
  ...
  priceDelta: z.coerce.number().int().min(0).max(MAX_CENTS).default(0),
});
...
export const patchOptionSchema = z
  .object({
    ...
    sublabel: z.string().trim().max(120).nullable().optional(),
    priceDelta: z.coerce.number().int().min(0).max(MAX_CENTS).optional(),
    status: z.enum(["ACTIVE", "INACTIVE"]).optional(),
    sortOrder: z.coerce.number().int().min(0).max(999).optional(),
  })
  .refine((d) => Object.keys(d).length > 0, { message: "At least one field is required" });
...
const planBase = {
  ...
  price: z.coerce.number().int().min(0).max(MAX_CENTS),
...
export const patchPlanSchema = z
  .object({
    ...
    price: z.coerce.number().int().min(0).max(MAX_CENTS).optional(),
